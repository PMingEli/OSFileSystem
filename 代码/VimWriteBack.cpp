...
bool VimWriteBack()
{
    ...
    else
    {
        ClearFile(vimroute);
        inode *currentInode;
        currentInode = OpenMutipleFile(vimroute);
        vimroute = "";
        ifstream t1("buffer.txt"); //读文件ifstream,写文件ofstream，可读可写fstream
        stringstream buffer;
        buffer << t1.rdbuf();
        string s = buffer.str();
        Write(*currentInode, s.c_str());
        // 1.检测文件是否达到了最大大小
        // int len_content = strlen(s.c_str());
